...
| OQ-26 | Plan identity on the presenter channel stays deferred (AD-10). Empty verse reference and lookup timeout fail closed (SCN-4). No projector → refuse lookup. Missing plan → return to Hub as UC-11. | Projector shows another Service; overlay hangs or has no screen. | 2026-08-19 | agent |
...
| OQ-28 | AD-16 is not listed on Hub or Presenter Inherited Constraints until the snapshot table ships (OQ-14). | Create/present is read as if it already cloned a freeze. | 2026-08-19 | agent |
...
| OQ-31 | Until FR-21 Admin delete HTTP ships, the SQL-delete proof path may leave `position` gaps; the list still `ORDER BY position`. | Contiguous-position invariant is unread against the only delete proof. | 2026-08-19 | agent |
