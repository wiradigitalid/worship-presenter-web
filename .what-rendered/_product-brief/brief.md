...
| OQ-58 | The Live Slide Preview's `slides` prop is dead weight: `/api/services/preview` returns `plan.DrawItem` (`{artifact, fade}`) while the SPA types that array as `SlidePreviewItem` (`{id, kind, title, subtitle, body}`) — the two shapes share no field. Assumed, until the owner rules: acceptable as-is, because OQ-57's fix moved the badge onto carried group metadata (`GroupRef.RoleLabel`) and `resolvePreviewTitle` already falls back to the entry label, so the two cells the Operator reads are correct. Not fixed here because a `DrawItem` -> `SlidePreviewItem` adapter is a wider change than the badge defect it was found behind. | The preview's body-text cell is empty on every API-served row and `legacyToneClass(slide?.kind)` never fires, so an Operator sanity-checking a rundown sees each slide's badge and title but never its text — and a type that claims a shape it never receives invites the next reader to trust `slide.title` again, which is exactly how OQ-57 hid. |  | 2026-08-22 | agent |
