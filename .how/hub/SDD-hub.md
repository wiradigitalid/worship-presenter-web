...
| --- | --- | --- | --- | --- | --- |
| POST /api/auth/login | Spinner until browser timeout | 500 | Wrong credentials → 401; lockout → 429 + Retry-After | Sign-in failed or wait message | Server error if 500 |
...
| GET /api/services | List does not appear | 500 page | Corrupt payload → page error | Slow: spinner then empty list. Absent: error page, not a silent empty Hub | console |
| POST /api/services | Timeout; the row may already exist if the client retries a 201 | 500 | Date exists without override → 409; empty/no date → 400; bad Rundown otherwise visible | 409: existing Service shown, no silent update. 400: no row | parser / NFR-5 |
...
| POST /api/webhook | Timeout at picoclaw | 503 if secret unset; silent if the agent is down | Wrong secret → 401; other JSON → 400 / parse visible | CAP-11 later: Events get no read-back; Operator sees Hub. Not this phase's handover | does not log the secret |
| /login | — | — | — | Login form | — |
| / | Slow list | 500 page | — | Empty list | — |
| /services/new | — | — | Parse failure visible | Form — this phase's create boundary (UC-2) | — |
...
| UC-17 | LC-8 `correct` — later CAP-11 | LC-12 | Service | read-back |
