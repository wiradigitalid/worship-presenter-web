...
| Validation | Translation code required, registry-validated; absent/unrecognised refused (AD-28). Empty `ref` → 400 (SCN-4). [PARTIAL] — `PresenterOperator` currently sends `ref` only; the route falls back to `DEFAULT_TRANSLATION`. |
...
| --- | --- | --- |
| Empty `ref` | 400 | Fail closed (SCN-4). Do not push overlay. Not a silent no-op. |
