...
2. The system deletes the Service, one-off announcement items, and local files in `UPLOADS_DIR` bound only to that Service.
...
That Service cannot be opened again. That week's local photo/flyer files are not left in `UPLOADS_DIR`. The recurring announcement list remains for other weeks.
