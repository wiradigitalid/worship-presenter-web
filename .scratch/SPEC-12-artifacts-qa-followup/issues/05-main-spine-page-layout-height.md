...
- [ ] Start with `wdi-systematic-debugging` — this has not been code-inspected yet, only reported
      (`defects.yaml` BUG-11).
- [ ] Trace the height/flex chain from the route shell down through the Main Spine editor layout to
      the Deck Sequence list container; find and fix the missing/broken `height`/flex constraint.
- [ ] Add a `min-height` floor on the outer editor container so the canvas stays usable on a very
      short viewport.
- [ ] Add a static structural guard test (`tests/artifact-editor-layout.test.mjs`, in
      ...
      drop them.
- [ ] Verify the actual visual result manually in the running app (`run` skill) — a rendered-layout
