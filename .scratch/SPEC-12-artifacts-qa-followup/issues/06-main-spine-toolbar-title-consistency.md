...
- [ ] Per `DEC-009` (the Main Spine "New Slide" panel's `+ Add` button's gray/low-contrast look was
...
- [ ] Per `DEC-010`: confirm the "New Slide" panel's wording already matches the accepted New/Add
...
- [ ] Per `DEC-011` (Main Spine's own instance): the title area reads `[Rename] | Canvas: [Reset]
