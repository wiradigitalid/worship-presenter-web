...
The as-built is one Docker Compose unit whose always-on process is the Go API, plus a Node binary only to exec the PPTX worker (DEC-003 / AD-4 / AD-30). **Do not run multiple API processes against the same database file.**
...
A production image copies shipped corpora. Never bake `data.db`. The image is Go + SPA assets + a Node binary for the PPTX child (OQ-35).
