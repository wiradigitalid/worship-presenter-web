...
| FR-25 | Epic 24 | Done — operator `ui_locale` is applied by `src/app/(operator)/layout.tsx`; the projected root does not read it |
...
### Epic 17: An operator surface that is readable and honest *(in-progress — Stories 17.1, 17.2, 17.3, 17.4, 17.6, 17.7, and 17.8 done; Story 17.5 in review; 17.9 backlog)*
...
So that it is not filed as *Create Next App*. `src/app/(operator)/layout.tsx` `metadata` now ships `title: "Worship Presenter Web"` and `description: "Operator hub for preparing and projecting a worship service."` (product-owned strings from `DESIGN.md` frontmatter). Regression: `tests/theme-chrome.test.mjs`.
...
**This story shipped no operator-visible change.** No file under `src/` or `tests/` is touched; `sonner` stays installed, `Toaster` stays mounted nowhere, and no `toast(` call was added. The wiring is `17-9-toast-channel-wiring`. Story 17.7 has since supplied the operator-scoped root AD-24 requires at `src/app/(operator)/layout.tsx`, so that later story's former structural dependency is satisfied.

#### Story 17.7: The Room-Facing Shell Belongs to the Route, Not to the App *(done — implemented and reviewed 2026-08-09)*
...
**This story took AD-24 from `[ADOPTED, partial]` to `[ADOPTED]`.** Its two same-change-set obligations were completed: the IA table in `EXPERIENCE.md` records both projected routes' ownership, and the spine amendment ran through `bmad-architecture` Update with AD-24's ID stable. `PresenterOperator` remains an operator surface under the sibling operator root; it did not gain a projected-shell colour parameter.
...
**Implemented 2026-08-09.** The chosen mechanism is now `src/app/(projected)/layout.tsx`, a Server-Component root that owns both unchanged projected URLs and literal black `html`/`body` first-paint claims. `not-found.tsx` and `error.tsx` live at the same boundary. All operator routes moved beneath the sibling `src/app/(operator)/layout.tsx`, which keeps the existing metadata/fonts/locale/provider contract and supplies the operator-scoped boundary Story 17.9 needs. The guard derives pages, layouts, special files, full-screen clients, props, and module roots structurally from the projected group; its async-default parser and per-document shell state are closed in the same change set. A Next 16.2.10 production build reports both public projected URLs unchanged and no conflicting-path failure.
...
#### Story 17.9: The Toast Channel Gets Wired *(backlog — Story 17.7 dependency satisfied)*
...
**Story 17.7 dependency satisfied 2026-08-09.** AD-24's narrowest-layout mount rule makes the mount decidable: the toast provider's consumers are operator routes only, never the projector or the slideshow, so `src/app/(operator)/layout.tsx` is the available mount and the sibling projected root cannot inherit it. A per-page mount across the operator-facing pages remains the leaf-widening pattern Story 17.8 closed the guard against, not a substitute for the route segment.
...
- **One component serves every operator surface** — Presenter Mode lookup and the create/edit service forms — on the shipped `HymnNumberAutocomplete` precedent (`src/components/HymnNumberAutocomplete.tsx`), already mounted at four sites in `src/app/(operator)/services/new/CreateForm.tsx` and four in `src/app/(operator)/services/[id]/EditForm.tsx`.
...
**What existed when measured on 2026-08-01 rather than estimated.** **No i18n infrastructure of any kind.** `lang="en"` in the layout now at `src/app/(operator)/layout.tsx` was the entirety of it. The surface was small and that was the point — 39 `.tsx` files, 26 client components, roughly 55 user-facing literals in JSX and attributes, plus about 158 message strings under `src/lib` and `src/app/api`, many of them developer-facing and not translatable. **Estimate 100–150 real strings.**
