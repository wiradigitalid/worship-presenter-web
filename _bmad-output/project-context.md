...
- **A client boundary mounts at the narrowest root or nested layout covering its consumers; the layout itself stays a Server Component.** `src/app/(operator)/layout.tsx` owns metadata, fonts, `getUiLocale()`, and the one `ThemeProvider` because those consumers are operator routes only. `src/app/(projected)/layout.tsx` is the sibling room-facing root and carries none of them. **Passing `children` through a client provider does not make them client components** — they arrive already server-rendered. To add a provider, enumerate its consumers and mount at the narrowest layout containing all of them. `'use client'` on either layout converts that layout — never do it. `<html suppressHydrationWarning>` belongs only to the operator theme boundary
- **Room-facing surfaces are closed to operator chrome (AD-24).** Projector, web slideshow and PPTX never read it, under any setting. Every browser route under `src/app/(projected)` inherits a server-rendered literal black `html`/`body` shell with hidden overflow and no reserved scrollbar gutter; its `not-found.tsx` and `error.tsx` are generic, literal-colour fallbacks. The projected tree paints literal colours (including `#FFFFFF`, `#000000`, `#0B1220` and `transparent`) or registry-resolved inline styles, never a theme token. A full-screen room-facing *client* surface also neutralises the shell defensively through **one shared implementation, never its own copy** — `src/lib/projected-shell.ts` keeps independent reference-counted state per `document` and is reached via `src/lib/use-projected-shell.ts`. The route-group layout owns server first paint; the hook owns only the hydrated defence
...
- **Projected guards derive their subjects from the route-group structure.** `tests/helpers/projected-routes.mjs` discovers pages, layouts, and all App Router special files under `src/app/(projected)` and normalizes route-group-free public URLs. `tests/theme-chrome.test.mjs` derives shell files, transitive module roots, full-screen clients, and component props from that discovery. Add a room-facing route or special file under the projected group; never add a second leaf inventory beside it
