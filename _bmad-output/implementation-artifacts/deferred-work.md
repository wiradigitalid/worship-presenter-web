...
| Debt | Owner | Why it can wait |
| --- | --- | --- |
| **AD-27's identity row still holds display columns.** `bible_books` remains `(id, name, short_name)` because SQLite cannot drop NOT NULL columns in place; names are written once (`ON CONFLICT DO NOTHING`) and live reads go to `bible_book_names`. Book ids are still supplied by the corpus file. | **Story 21.4 remainder / 21.5** | Display arbitration is closed. Dropping the leftover columns is a rebuild migration, not a second translation hazard. |
...
| **AD-28's one-matcher clause should ship asserted.** Corpus files that carry an `aliases` field are refused. Remaining: a source scan that no unkeyed alias list survives, and Story 21.5's suggestion endpoint. | **Story 21.5** | Runtime match/ambiguity tests live with the matcher; a green suite is not over-read as covering the suggestion surface. |
