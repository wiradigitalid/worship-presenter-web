...
| UI-locale switcher | `/admin` | Sets the language the operator interface is written in, and `<html lang>` with it. **It reaches no room-facing surface** — there is no `projection_locale`, by decision (PRD §4.12), so this is AD-24's closure read in the other direction. Landed on by Flow 9 | **Shipped 2026-08-02** (`admin/UiLocaleSettings.tsx` → `PUT /api/admin/settings`, Story 24.1). Two locales, `en` and `id`; the catalogue is **code that ships with the build** (`src/lib/i18n/`, AD-25), so the switcher offers what this build has catalogues for and never what a corpus declares — the two vocabularies are separate constants on purpose. `<html lang>` reads the setting in the root layout, and `force-dynamic` there is what keeps it out of the build: `/login` was the one page statically generated, and it would have frozen `en` on exactly the route the setting could not reach. **The closure is asserted, not assumed** — `tests/i18n.test.mjs` walks the projected module tree and fails on a catalogue import or a `getUiLocale` call; `<html lang>` on the room-facing routes is the one deliberate exception, because it paints nothing |
...
The owner ratified a **combined inline + toast** design for this channel on 2026-08-05, closing the question the `sonner` row below and *Open Item 4* had carried since 2026-07-31: this product does want a transient channel — under a rule, not none, and not the delete-two-rows-and-uninstall alternative that had been the likely outcome. **Nothing here is a discovery.** Clause three below is already the second sentence the `sonner` row has carried since it shipped — the combined design is what these documents described all along; what was missing was never the design, only the wiring and a rule for when each channel applies. The wiring is `17-9-toast-channel-wiring`, dependent on Story 17.7 (`ARCHITECTURE-SPINE.md` AD-24 — the toast provider's consumers are operator routes only, never every route, so the root layout is the wrong mount and an operator-scoped route segment is the right one). Story 17.7's own registered contract is one route-group layout owning every *room-facing* URL — the room-facing half of the split, not a promise of the operator-scoped half `17-9` needs; if 17.7 leaves the operator routes directly under `src/app`, `17-9` creates that segment itself rather than wiring into one 17.7 already built. This decision is recorded once, here; the `sonner` rows below and in [`DESIGN.md`](./DESIGN.md), and *Open Item 4*, cite this block rather than restating a clause.
...
| `ThemeToggle` | Cycles **system → light → dark** and back, one click per step, persisting to `localStorage` so the choice survives a reload and a new tab. With nothing stored the hub follows the operating system. `system` stays in the cycle deliberately: it is what a first visit gets, and a two-way switch would make it unreachable after the first click. **Scope is operator chrome only** — the presenter and slide-grid surfaces pin their own dark surface and ignore the choice (a dim sanctuary is not a preference), and no theme the operator picks can reach a projected slide, a slideshow frame or a downloaded PPTX. That last is a requirement, not a side effect. **It took two attempts to be true.** The first pass verified it node by node inside the slideshow's own tree — 14 of 14 identical — and that verification could not see `html` or `body`, which is where the theme actually paints: `body` carries `bg-background`, `html` reserves a scrollbar gutter, and a `fixed inset-0` surface sizes to the viewport *minus* that gutter, so the shell showed as a strip down the edge of the projected screen. Permanently white before a theme existed; following the operator's live choice after, because next-themes syncs across same-origin windows. The projector had neutralised this for itself from the start; `useProjectedShell` now does it for both surfaces, and the route shells' error states paint in literal colours for the same reason. **A second nuance, and one half of it was not latent at all:** `globals.css` applies `border-border` *and* `outline-ring/50` through a universal selector, so every node in the projected tree computes a theme-dependent border **and** outline colour. The border paints nothing while Tailwind's preflight leaves every border at `0px` — that edge is one width utility away and no closer. The **outline** was different: the UA supplies the width itself on `:focus-visible`, so focusing the slideshow's *Exit* link or either recovery link drew a ring in the operator's theme, and the measurement that cleared this surface was taken with nothing focused. Those three focusables now state `focus-visible:outline-white`. **Where the guarantee stops:** it covers the projected *tree* — the slides, the client surfaces, and the route shells' **own elements**, which paint in literal colours and carry no theme token. It does **not** cover the *shell behind* them, and that is **four** holes with one root, not three: the root layout; the server's first paint, before any hook runs; a `notFound()` at a room-facing URL; and **the two route shells' own `html`/`body`** — they are Server Components, so neither can call `useProjectedShell()` at all, and the reserved `scrollbar-gutter: stable` still shows the themed `body` as a strip down the edge of the branch a registry failure faces the congregation with. Earlier wording here said the guarantee covered "both route shells" and then listed three holes; the fourth is the one AD-24's `[ADOPTED, partial]` tag was created for, and a reader of this row alone would have concluded the shells were closed. All four are **Story 17.7's** contract, not this one's, and AD-24 stays `[ADOPTED, partial]` until 17.7 closes it. `tests/theme-chrome.test.mjs` guards what is in scope: no theme token and no edge width in either route shell or client tree, no `className` reaching the projected wrapper from outside (the parameter is gone from both signatures, so it is a compile error now), a literal outline **colour** on every focusable in the projected **tree** — both axes swept, the tags by walking the JSX and the files by the same transitive module walk the token guard uses, because widening the tags and leaving the files a six-entry list meant a focusable in a component a projected client imports was still unchecked; `outline-none`, `outline-0`, `outline-transparent` and `outline-inherit` are all rejected — the first two delete the ring this exists to protect, and the last two do the same thing by making it invisible or by inheriting the very `--ring` colour it exists to keep out; `text-white` on every full-screen branch root; both surfaces holding the shell at literal black — and the claim itself exercised against a document stub, set *and* restored, rather than matched as source text. |
...
- **`<html lang>` follows the setting now, and that is not yet an accessibility win.** Story 24.1 (2026-08-02) replaced the hard-coded `en`; the mechanism is right and was verified in served markup on every route, including the projector and `/login`. But until the Story 24.2 sweep the hub is bilingual in **one admin block**, so choosing Indonesian makes the document declare `id` while 100–150 strings are still English literals. **A screen reader will then read English text with Indonesian phonetics** — a sharper misstatement than the honest `en` it replaced, not a softer one. Recorded here because the epic was split deliberately, and the mismatch between declared and actual language is what the split costs; this bullet is the only line on the list with a dated owner, and the mismatch closes when 24.2 lands.
